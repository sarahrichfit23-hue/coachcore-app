...
function checkEnvFile() {
  ...
  if (fs.existsSync(envPath)) {
    ...
    log(
      '\nThis is the most common cause of "Database connection error"',
      colors.yellow,
    );
...
function checkRequiredVars() {
  ...
  for (const varInfo of requiredVars) {
    ...
    if (value && value.trim() !== "") {
      log(
        `✓ ${varInfo.name.padEnd(20)} - ${varInfo.description}`,
        colors.green,
      );
    } else {
...
function checkDatabaseUrl() {
  ...
  if (dbUrl.includes(":6543")) {
    issues.push(
      "Port 6543 is deprecated. Use port 5432 for direct connections",
    );
  }
  ...
  if (dbUrl.includes("YOUR_PASSWORD") || dbUrl.includes("YOUR_HOST")) {
    issues.push(
      "URL contains placeholder values - replace with actual credentials",
    );
  }
...
function checkJwtSecret() {
  ...
  if (secret.length < 32) {
    issues.push(
      `Secret is too short (${secret.length} chars). Should be at least 32 characters`,
    );
  }
